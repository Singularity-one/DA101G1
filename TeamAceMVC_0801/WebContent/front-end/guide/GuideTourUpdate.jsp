...
pageContext.setAttribute("guideTourVO", guideTourVO);
	...
	<div class="container">
		...
		<div class="row justify-content-center content">
			...
			<form method="post" action="GuideTour.do" onsubmit="return false" id="uploadForm" enctype="multipart/form-data">
				<p class="title">EDIT YOUR TOUR</p>
					...
					<div class="col-4">
					 ...
					 <div class="col-8">
						<label for="tourName">伴遊行程名稱</label><span id="nameError" style="color:red;font-size:9;margin-left:22px;"></span>
	 			 	    	<input type="text" id="tourName" name="guideTour_name" class="form-control" value="${guideTourVO.guideTour_name}">
					</div>
				</div>
				<div class="form-group form-row">
					<div class="col-4">
						<label for="tourPeo">人數上限</label><span id="peoError" style="color:red;font-size:9;margin-left:22px;"></span>
	 			 	      	<input type="text" id="tourPeo" name="guideTour_peo" class="form-control" placeholder="最多99人" value="${guideTourVO.guideTour_peo}">
					</div>
					 <div class="col-4">
				...
				<div style="display: flex;margin-top:40px">
					<div id="land-div" style="width:320px;height:550px">
						...
						<div class="more-land" id="more-land" style="display: none;">
							<i class="fa fa-angle-double-down" style="margin-left: 3px"></i><br>
							<i class="fa fa-angle-double-down" style="margin-left: 3px"></i>
							<select class="custom-select custom-select-sm" id="trans" name="transport" style="width:95px">
				...
				<div style="margin:40px 0;">
					<span class="sm-title"><label for="tourDetail">行程詳細內容</label></span><span id="detailError" style="font-size:9;color:red;margin-left: 20px"></span>
	 			 	<textarea class="form-control" id="tourDetail" name="guideTour_detail" rows="8" placeholder="上限1000字" value="${guideTourVO.guideTour_detail}"></textarea>
 			 	</div>
 			 	<!-- 分類選項 -->
			 	...
			 	<div class="submit" style="width:100%;display: flex;justify-content: center">
			 		...
			 		<input type="hidden" name="guideTour_no" value="${param.guideTour_no}">
			 		<input type="hidden" name="jsonStr" id="jsonStr">
			 		<input type="hidden" name="guide_no" value="${guideVO.guide_no}">
			   		<button type="submit" class="btn" onclick="verify()">送出</button>
			 	</div>
			 </form>
			</div>
...
<script>
	...
	$(document).ready(function(){
		...
		//舊資料
		document.getElementById('area').value='${guideTourVO.guideTour_area}';
		var landLength=0;
		var trans;
		var hour;
		var min;
		<c:forEach items="${gtListSvc.getGTListByGTNo(guideTourVO.guideTour_no)}" var="gtListVO" varStatus="status"> 
			var landName ='${landscapeSvc.getOneLas(gtListVO.landscape_no).landscape_name}';
			var num = parseInt('${status.index}',10);
			var index = num +1;
			...
			addMarker(landName,lat,lng);
			selectLand['land'+index] = landName;
			if(index == 1){
				trans = '${gtListVO.landscape_trans}';
				hour= '${gtListVO.landscape_hr}';
				min= '${gtListVO.landscape_min}';
				$('#land1').val(landName);
			}
			if(index > 1){
				...
				$(newLand).find('#land').attr('id','land'+index);
				$(newLand).find('#land'+index).val(landName);
				$(newLand).find('#trans').val(trans);
				$(newLand).find('#hour').val(hour);
				$(newLand).find('#min').val(min);
				var landInput = $(newLand).find('#land'+index);
		...
		document.getElementById('tourDetail').value=`'${guideTourVO.guideTour_detail}'`;
		var catArray = document.getElementsByName('category');
		<c:forEach var="gtcListVO" items="${gtcListSvc.getCategoryByGTno(guideTourVO.guideTour_no)}">
			var cat = '${gtcListVO.category}';
			for(var i=0;i<catArray.length;i++){
				if(catArray[i].value == cat){
					catArray[i].checked = true;
				}
			}
		</c:forEach>
	...
	function verify(){
		...
		if(tourName.value!="" && tourPeo!="" && parseInt(peoValue, 10)<=99 && tourdetail.length<=1000){
			uploadForm.onsubmit=true;	
		}
	}
</script>
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyANKHSWauEfGG_DUy-MEfvdocMH1e1KrMI&callback=initMap"></script>
    <!-- Optional JavaScript -->
