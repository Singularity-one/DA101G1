...
<%
	...
	List<Itinerary_trackVO> ittlist = ittSvc.getMemberItinerary_track(paramMem_no);
	List<Itinerary_detailVO> itdlist = new ArrayList<Itinerary_detailVO>();
	for (Itinerary_trackVO ittlist2 : ittlist) {
		Itinerary_detailService itdSvc = new Itinerary_detailService();
		itdlist.add(itdSvc.getOneItinerary_detail(ittlist2.getItinerary_no()));
	}
	pageContext.setAttribute("itdlist", itdlist);
%>
...
#map-canvasQ{
	height: 240px; /* The height is 400 pixels */
	width: 250px; 
}
...
<script>
		...
		function initMap() {
			...
			var mapLatLng = new google.maps.LatLng({lat: 23.81749, lng: 120.88567}); // 緯度経度のデータ作成
...
<!-- 				</div> -->
		...
		<div class="row justify-content-center content">
			...
			<div class="btngroup col-7 container">
				...
				<div class="brightness col-2" id="brightness"  onclick="tripGame()">
					<img src="images/car.png" class="btnimg"  >
				</div>
				<div class="brightness col-2" id="brightness" onclick="track()">
...
<!-- 							<i class="fa fa-home"></i>&nbsp 新潟県 <br> <br> -->
					...
					<div class="col-12 photowall" >
							<div id="tripGametable" style="display:none">
								...
								<div class="itstatus"><b class="Starting">進行中</b></div>
									<sapn class="itinerarySelect"  onclick="itineraryQuestionShow('${itineraryStartVO.itinerary_no}','${param.mem_no}','${memberVO.mem_no}')">							
		...
		<div id="track" style="display:none">
            <c:forEach var="itinerary_detailVO" items="${itdlist}">
            <div  id='${itinerary_detailVO.itinerary_no}' class="row" style="border:1px solid #e0e0e0;margin:10px;padding:5px;width:610px;height:140px;">
                    <div class="col-md-4">
                    
                    <img width="50%" src="ItdReader.do?itinerary_no=${itinerary_detailVO.itinerary_no}"></div>
                    
                    <div><b>
						<c:forEach var="itinerary_detailVO" items='${itinerary_detailSvc.getLands(itinerary_detailVO.getItinerary_no())}' varStatus="s">      
...
<!-- 						<i class="fa fa-arrow-right"></i> -->
								 ...
								 <br>
								 <span class="ratings">
<!-- 								  <a href="#" title="1/5"><i class="rate rated fa fa-star"></i></a> -->
<!-- 								  <a href="#" title="2/5"><i class="rate fa fa-star"></i></a> -->
<!-- 								  <a href="#" title="3/5"><i class="rate fa fa-star"></i></a> -->
<!-- 								  <a href="#" title="4/5"><i class="rate fa fa-star"></i></a> -->
<!-- 								  <a href="#" title="5/5"><i class="rate fa fa-star"></i></a> -->
								</span>
							
								<div class="adjust"  style="display:flex;  text-align:right; width:350px; justify-content:flex-end">
								<input  type="button"  value="取消收藏"  onclick="cancelTrack('${itinerary_detailVO.itinerary_no}','${param.mem_no}')"  class="btn-info">
								</div>
...
// 		$('#tripGametable').append('<div></div>')
	...
	function itineraryQuestionShow(itinerary_no,param_memNo,memNo){
			console.log(itinerary_no)
			console.log(param_memNo)
			console.log(memNo)
			$('#itineraryQuestion').attr('style','')
			$('#sendAnswer').attr('disabled','')
			$('#sendLocation').attr('disabled','')
					...
					var random =  Math.floor(Math.random()*(data.length));
 					$(data).each(function(i, item){
 						...
 						if(item.question!=null){
 							...
 							$('#itineraryQuestion').append("<b>1.回答問題:</b>"+"<b id=quest>"+item.question+"</b><br>"+
//  									"<b >剩餘時間:</b><span id=countdown></span><br>"+
 							...
 							"<button id=sendAnswer type=button class='btn btn-warning' onclick=isAnswer("+"'"+item.answer+"'"+")>送出</button><br>");
 						}
 						
 						if(random==i){
								$('#itineraryQuestion').append("<b>2.到"+item.landscape_name+"定位</b><br><br>"+
								"<button id=sendLocation type=button class='btn btn-warning' onclick=getLocation()>定位檢查</button>");
						}
 						$('#itineraryQuestion').append('<div id="map-canvasQ"></div>');
 						
 					});
 					
//  					initial(itinerary_no,param_memNo);	
...
// 					}
	...
	function cancelTrack(itiNo,memNo){
		$('#'+itiNo).hide();
		$.ajax({
			 type: "GET",
			 url: "Itinerary_trackServlet.do",
			 data: creatQueryString(itiNo,memNo),
			 dataType: "json",
			 success: function (data){
				 clearSelect();
				$(data).each(function(i, item){
					console.log(item.deleteTrack);
				});
				swal({
		            title: "確定取消收藏？",
		            html: "",
        ...
        function creatQueryString(itiNo,memNo){
			var queryString= {"action":"deleteFromItinerary_track","itinerary_no":itiNo,"mem_no":memNo};
			return queryString;
		}
...
// 				alert('恭喜答對了');
				swal({
	                ...
	                if (result) {
	                	if(answer===$(':checked').attr('id')){
// 	                    	swal("答對嚕!<br>獲得50點", "", "success");
	                    	$('#quest').append('&nbsp&nbsp&nbsp<i class="fa fa-check-circle"></i>');
	                	}else{
	                		swal("答錯嚕!", "", "error"); 
// 	                		<div style="padding:10px;background-color:green;opacity:0.1;">
	                		$('#itineraryQuestion').attr('style','padding:10px background-color:black;opacity:0.5')
	                		$('#sendAnswer').attr('disabled','disabled')
	                		$('#sendLocation').attr('disabled','disabled')                		
	                		$('#quest').append('&nbsp&nbsp&nbsp<i class="fa fa-times-circle"></i>');
	                	}
	                }
	            }, function(dismiss) { // dismiss can be "cancel" | "overlay" | "esc" | "cancel" | "timer"
	            		swal("取消", "", "error");
...
// 		  alert('經緯度：' + latitude + ', ' + longitude);
		...
		function getLatLng() {
		    ...
		    var map;
	         mapOptions = {
	          center: { lat: 24.9774721, lng: 121.1824813},
	          zoom: 13
	        };
	         map = new google.maps.Map(
	            document.getElementById('map-canvasQ'),
	            mapOptions);			
	         
	         markerLatLng = new google.maps.LatLng({lat: 24.9774721, lng: 121.1824813});
...
// 	               					console.log('ok');
	                       ...
	                       function creatQueryString(itiNo,memNo){
	               			var queryString= {"action":"addPoint","pointafter":"50"};
	               			return queryString;
	               		}
